...
- a declared post-decision lower bound agrees with the inner savings grid: the
  ...
  discharges such a declaration through an explicit structural proof, so a
  number the grid does not carry has to be refused before that proof applies.
...
def validate_negm_regime(
    *,
    ...
    _fail_if_required_grid_withholds_its_points(
    ...
    # would still discharge a declared bound as one the grid guarantees, on
    # the strength of its shape, with nothing having compared the two numbers.
    fail_if_declared_lower_bound_disagrees_with_the_grid(
        regime_name=regime_name,
        user_regime=user_regime,
        solver=inner,
...
def _fail_if_required_grid_withholds_its_points(
    *, regime_name: RegimeName, user_regime: UserRegime, solver: _BoundNEGM
) -> None:
    """Require construction-time nodes only for grids the NEGM build reads.

    The outer state nodes lay out the continuation, the inner savings nodes
    define the inversion, and the outer search nodes define the adjuster
    candidates. Other state and action grids can still receive their points
    through params because the nested build does not lift their nodes into one
    of those three constructions.
    """
    outer_state_grid = _solve_grids(slot=user_regime.states)[solver.outer_state]
    required = (
        (f"outer state grid '{solver.outer_state}'", outer_state_grid),
        ("inner savings grid", solver.inner.savings_grid),
        ("outer search grid", solver.outer_grid),
    )
    for role, grid in required:
        fail_if_grid_withholds_its_points(
            grid=grid,
            role=role,
            regime_name=regime_name,
