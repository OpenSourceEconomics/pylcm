...
def _build_Q_and_F_per_period(
    *,
    ...
    def group_key(period: int) -> tuple[tuple[RegimeName, ...], Hashable]:
        ...
        # The perceived (solve) continuation pool is resolved per period below, so
        # periods whose continuation functions resolve to different closures must NOT
        # share a kernel — fold its per-period signature in too (round-11 F1). `None`
        # (solve phase / no specialization) contributes a constant, collapsing the
        # grouping.
