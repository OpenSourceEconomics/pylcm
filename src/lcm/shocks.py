...
class Uniform_Shock:
    ...
    def init(self, n: int, x_min: float, x_max: float):
        self.values, self.transition_probs = discretized_uniform_distribution(n, x_min, x_max)

class IID_Shock:
    values: Array
    transition_probs: Array

    def init(self, n: int, mu_eps: float, sigma_eps: float, n_std: int =3):
        self.values, self.transition_probs = discretized_uniform_distribution(n, mu_eps, sigma_eps, n_std)

class AR1_Shock:
    values: Array
    transition_probs: Array

    def init(self, method: str, n: int, rho: float, sigma_eps: float, mu_eps: float =0., n_std: int = 1):
        if method == "tauchen":
            values, transition_probs = tauchen(n=n,rho=rho, sigma_eps=sigma_eps, mu_eps=mu_eps, n_std=n_std)
            self.values = values
            self.transition_probs = transition_probs
        elif method == "rouwenhorst":
            values, transition_probs = rouwenhorst(n=n,rho=rho, sigma_eps=sigma_eps, mu_eps=mu_eps, n_std=n_std)
            self.values = values
            self.transition_probs = transition_probs
        else:
            raise ShockInitializationError(
            f"The requested discretization method does not exist.",
            )


def discretized_uniform_distribution(n: int, x_min: float, x_max: float ):
    return jnp.linspace(start=x_min, stop=x_max, num=n), jnp.full((n,n), fill_value=1/n)


def discretized_normal_distribution(n: int, mu_eps: float, sigma_eps: float, n_std: int =3):
...
def tauchen(n: int, rho: float, sigma_eps: float, mu_eps: float =0., n_std: int =2):
    ...
    # demeaned y_t
    P = _fill_tauchen(x, P, n, rho, sigma_eps, half_step)
...
def _fill_tauchen(x, P, n, rho, sigma, half_step):
    for i in range(n):
        ...
        for j in range(1, n - 1):
            z = x[j] - rho * x[i]
            P = P.at[i,j].set((cdf((z + half_step) / sigma) -
                       cdf((z - half_step) / sigma)))
    return P
            
def rouwenhorst(rho: float,sigma_eps: float, n: int, mu_eps: float = 0., n_std=2 ):
...
print(discretized_normal_distribution(5, 3, 0.5))