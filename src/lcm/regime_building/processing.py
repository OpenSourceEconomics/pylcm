...
def process_regimes(
    *,
    ...
    Before any downstream machinery runs, `detect_model_partitions`
    identifies each `DiscreteGrid` state whose `dispatch` is
    partition-lifted (`DispatchStrategy.PARTITION_SCAN` or
    `PARTITION_VMAP`) and lifts it out of the regime via
    `lift_partitions_from_regime`, storing it on the returned
    `InternalRegime` as `partitions`. Solve and simulate then iterate
    over the product of partition grids once per point — the reduced
    regime below never sees a partition state as a vmap'd axis.

    Default-dispatch (`FUSED_VMAP`) states whose transition is `None`
    are **not** partition-lifted: they stay in the state-action space
    with an auto-generated identity transition (generated by
    `collect_state_transitions`). ShockGrid transitions are generated
    from the grid's intrinsic transition logic. Regime transitions come
    from `regime.transition`.
