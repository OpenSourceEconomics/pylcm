"""Collect state transitions from user-facing `state_transitions` mappings.

Two collectors, both walking a regime's `state_transitions`:

- `collect_state_transitions` — every state's transition *function* (bare
  callable, `MarkovTransition`, an identity for `None`, per-target dict).
- `collect_stochastic_state_transitions` — validation *metadata* for the
  `MarkovTransition` entries only: a process-time AST subscript-order check
  plus `n_outcomes` derivation, cached on the canonical `Regime` and consumed
  by the pre-solve state-transition validator.
"""
