...
class Model:
    ...
    def solve(
        ...
        *,
        ...
        # Stack partition scalars as leading axes on the params pytree. A
        # single `jax.vmap` inside compile_solve will sweep the kernel over
        # that axis, so we pay the compile cost once and dispatch once.
        stacked_params, partition_shape = stack_partition_scalars(
        ...
        compiled = compile_solve(
            internal_params=stacked_params,
            ages=self.ages,
            internal_regimes=self.internal_regimes,
            logger=logger,
        ...
        try:
            # Validate regime transitions once per partition point (the
            # validator still expects per-point scalars). Validation is
            # logical (not compute-path) and always cheap relative to solve.
            for partition_point in iterate_partition_points(
                partition_grid=self._partition_grid
            ):
                validate_regime_transitions_all_periods(
                    internal_regimes=self.internal_regimes,
                    internal_params=inject_partition_scalars(
                        internal_params=internal_params,
            ...
            raise
        period_to_regime_to_V_arr = _reshape_leading_partition_axis(
            raw=raw, partition_shape=partition_shape
        )
...
def _reshape_leading_partition_axis(
    *,
    raw: MappingProxyType[int, MappingProxyType[RegimeName, FloatND]],
    partition_shape: tuple[int, ...],
) -> MappingProxyType[int, MappingProxyType[RegimeName, FloatND]]:
    """Expand the flattened leading partition axis into one axis per partition.

    `compile_solve` vmaps over a single flat axis of size
    `prod(partition_shape)`; this helper reshapes every V-array so the
    partition dimensions become separate leading axes — restoring the
    canonical "partitions > state axes" convention.

    The reshape uses JAX's row-major (C-order) semantics, which must
    match the order in which `stack_partition_scalars` laid out the
    partition points (row-major via `itertools.product`).

    When `partition_shape` is empty the input is returned unchanged.
    """
    if not partition_shape:
        return raw
    expected = math.prod(partition_shape)
    reshaped: dict[int, MappingProxyType[RegimeName, FloatND]] = {}
    for period, regime_to_V in raw.items():
        reshaped[period] = MappingProxyType(
            {
                name: _reshape_one_V(
                    V=V, partition_shape=partition_shape, expected=expected
                )
                for name, V in regime_to_V.items()
            }
        )
    return MappingProxyType(reshaped)
...
def _reshape_one_V(
    *,
    ...
    """Reshape a single V-array from a flat leading axis to `partition_shape`.

    Raises AssertionError if the leading-axis invariant is violated —
    `V.shape[0]` must equal `prod(partition_shape)`. This protects the
    row-major contract between `stack_partition_scalars` and this
    reshape: if either side ever diverges, misattributed values would
    silently propagate; the assertion stops that at the boundary.
    """
    if V.shape[0] != expected:
        msg = (
            f"V-array leading axis ({V.shape[0]}) does not match "
            f"prod(partition_shape)={expected}. The row-major contract "
            f"between iterate_partition_points / stack_partition_scalars "
            f"and this reshape is violated."
        )
        raise AssertionError(msg)
    return V.reshape(partition_shape + V.shape[1:])
