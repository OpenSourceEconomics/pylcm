...
`sqrt(wealth)` is strictly positive at every reachable wealth.

See docs/examples/epstein_zin.md for a full exposition of the model,
...
_LAST_ACTIVE_AGE: int = 80
_DEFAULT_CE: CertaintyEquivalent = PowerCertaintyEquivalent()
...
def get_model(
    *, certainty_equivalent: CertaintyEquivalent | None = _DEFAULT_CE
) -> Model:
    ...
    alive = Regime(
        ...
        active=lambda age: age < _LAST_ACTIVE_AGE,
    )
    dead = Regime(
        transition=None,
    ...
    return Model(
        ...
        ages=AgeGrid(start=60, stop=80, step="Y"),
