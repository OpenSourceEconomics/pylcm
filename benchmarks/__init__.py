...
- `track_*` — return an arbitrary scalar metric (e.g.
  `track_warmup` returns the JIT compilation time captured in
  `setup`).
...
1. `setup` calls the operation once and stores the elapsed time in
   `self._warmup_time`.  After this call all relevant JAX traces are cached.
2. `time_*` / `peakmem_*` then measure post-compilation performance.
3. `track_warmup` reports the first-call time so we can track compilation cost
   separately.
