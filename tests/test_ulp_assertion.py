...
def test_a_finite_value_does_not_agree_with_an_infinity():
    ...
    with pytest.raises(AssertionError):
        assert_agrees_to_ulp(got=np.array([np.inf]), expected=np.array([1.0]), n_ulp=16)
...
def test_opposite_sign_infinities_do_not_agree():
    ...
    with pytest.raises(AssertionError):
        assert_agrees_to_ulp(
            got=np.array([np.inf]), expected=np.array([-np.inf]), n_ulp=16
        )
...
def test_a_nan_does_not_agree_with_a_finite_value():
    ...
    with pytest.raises(AssertionError):
        assert_agrees_to_ulp(got=np.array([np.nan]), expected=np.array([1.0]), n_ulp=16)


def test_matching_infinities_agree():
    """Two identical infinities are the same answer."""
    assert_agrees_to_ulp(
        got=np.array([np.inf, -np.inf]), expected=np.array([np.inf, -np.inf]), n_ulp=0
    )
...
def test_finite_entries_are_still_compared_in_ulp():
    ...
    assert_agrees_to_ulp(got=np.nextafter(expected, np.inf), expected=expected, n_ulp=1)
