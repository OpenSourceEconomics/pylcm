...
# Matches the `dcegm` run's exogenous savings grid exactly. Its uniform
# spacing under-resolves the sharply curved retiree value function near the
# borrowing limit (in the reference implementation and in pylcm alike); the
# fixture comparison keeps it for comparability, and
# `build_dcegm_model(savings_grid=...)` accepts a clustered grid where the
# low-wealth region matters.
