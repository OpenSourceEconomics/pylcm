"""One-asset NBEGM toy whose cliff threshold lives inside a `MappingLeaf` param.
...
the ACA model groups its bracket/FPL schedules. The NBEGM ride-along solver must
    ...
    *,
    ...
    Args:
        variant: `"brute"` drives the alive regime by `GridSearch`; `"nbegm"` by
            the `NBEGM` schedule solver, which must follow the
