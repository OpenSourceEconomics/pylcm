"""BQSEGM ride-along toy with a stochastic multi-target lifecycle transition.
...
def build_model(
    *,
    ...
    Args:
        variant: `"brute"` drives both living regimes by `GridSearch`; `"bqsegm"`
            by the `BQSEGM` schedule solver, which must take the continuation as a
