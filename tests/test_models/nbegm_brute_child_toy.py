"""NBEGM regime whose continuation reads a separate brute-solved living child.

The M1 ACA slice solves one regime by `NBEGM` while every other living regime
stays on brute force, and the NBEGM regime transitions *into* those brute
children. This toy isolates that topology: a ride-along `young` regime (solved by
`NBEGM` or, in the reference, by `GridSearch`) transitions deterministically to a
distinct `old` regime that is always brute-solved, which in turn transitions to
the terminal `dead` regime. The NBEGM `young` solve must read `old`'s brute value
    ...
    *,
    ...
    Args:
        young_variant: `"brute"` drives `young` by `GridSearch` (the reference);
            `"nbegm"` by the `NBEGM` schedule solver, whose continuation must
        ...
        n_savings: Post-decision savings grid size (NBEGM path).
        savings_max: Upper bound of the savings grid (NBEGM path).
