"""One-asset NBEGM toy mixing a jump and a kink on two distinct income vars.
...
sits below the kink in one slice and above it in the other. NBEGM must merge the
    ...
    *,
    ...
    Args:
        variant: `"brute"` drives the alive regime by `GridSearch`; `"nbegm"` by
            the `NBEGM` schedule solver, which must merge the kink and the jump —
