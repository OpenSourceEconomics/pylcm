"""One-asset NBEGM toy whose budget kinks on two distinct derived income vars.
...
different liquid points and *reorder* between the `kind` slices. The NBEGM
    ...
    *,
    ...
    Args:
        variant: `"brute"` drives the alive regime by `GridSearch`; `"nbegm"` by
            the `NBEGM` schedule solver, which must merge the two derived-income
