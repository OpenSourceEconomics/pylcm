...
def test_get_func_indexing_params_multi() -> None:
    assert _get_func_indexing_params(func=_good_multi) == ["period", "health"]


def test_get_func_indexing_params_single() -> None:
    assert _get_func_indexing_params(func=_good_single) == ["health"]


def test_get_func_indexing_params_three() -> None:
    assert _get_func_indexing_params(func=_good_three) == ["period", "work", "partner"]
...
def test_get_func_indexing_params_detects_actual_order() -> None:
    ...
    result = _get_func_indexing_params(func=_swapped)
    assert result == ["health", "period"]
...
def test_no_subscript_returns_empty() -> None:
    ...
    assert _get_func_indexing_params(func=_no_subscript) == []
...
def test_scalar_function_returns_empty() -> None:
    assert _get_func_indexing_params(func=_no_probs_param) == []
...
def test_non_probs_array_param() -> None:
    """Works with array param names other than probs_array."""
    result = _get_func_indexing_params(func=_custom_array)
...
def test_non_probs_array_param_actual_order() -> None:
    ...
    result = _get_func_indexing_params(func=_custom_array_wrong_order)
    assert result == ["health", "period"]
