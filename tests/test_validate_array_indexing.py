...
from lcm._utils.error_handling import _get_func_indexing_params
...
def test_get_func_indexing_params_multi() -> None:
    assert _get_func_indexing_params(
        func=_good_multi, array_param_name="probs_array"
    ) == ["period", "health"]


def test_get_func_indexing_params_single() -> None:
    assert _get_func_indexing_params(
        func=_good_single, array_param_name="probs_array"
    ) == ["health"]


def test_get_func_indexing_params_three() -> None:
    assert _get_func_indexing_params(
        func=_good_three, array_param_name="probs_array"
    ) == ["period", "work", "partner"]
...
def test_get_func_indexing_params_detects_actual_order() -> None:
    ...
    assert _get_func_indexing_params(func=_swapped, array_param_name="probs_array") == [
        "health",
        "period",
    ]
...
def test_aliased_variable_detected_by_ast() -> None:
    ...
    with pytest.raises(ValueError, match="not function parameters"):
        _get_func_indexing_params(
            func=_aliased_variable, array_param_name="probs_array"
        )
...
def test_no_subscript_returns_empty() -> None:
    ...
    assert (
        _get_func_indexing_params(func=_no_subscript, array_param_name="probs_array")
        == []
    )
...
def test_scalar_function_returns_empty() -> None:
    assert (
        _get_func_indexing_params(func=_no_probs_param, array_param_name="nonexistent")
        == []
    )
...
def test_non_probs_array_param() -> None:
    ...
    assert _get_func_indexing_params(
        func=_custom_array, array_param_name="wage_grid"
    ) == ["period", "health"]
...
def test_non_probs_array_param_actual_order() -> None:
    ...
    assert _get_func_indexing_params(
        func=_custom_array_wrong_order, array_param_name="wage_grid"
    ) == ["health", "period"]
...
def test_dict_subscript_not_confused_with_array() -> None:
    ...
    assert _get_func_indexing_params(
        func=_dict_subscript, array_param_name="probs_array"
    ) == ["period"]
...
def test_multiple_subscripts_raises() -> None:
    """Multiple subscripts of the same array raise ValueError."""
