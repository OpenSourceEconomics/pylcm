"""Configure the heavy Mahler GPU workload from measured runner capacity."""
...
def create_mahler_gpu_model(*, report_path: Path) -> Model:
    """Build the workload on one explicitly selected addressable GPU."""
    config = create_mahler_execution_config(
        devices=tuple(jax.local_devices()[:1]), report_path=report_path
    )
    return create_model(execution_config=config)
...
def create_mahler_execution_config(
    *, devices: tuple[jax.Device, ...], report_path: Path
    ...
    The half factor is this workload's configuration policy. Compiler admission
    ...
    receipt: dict[str, Any] = {
        ...
        "formula": (
            "min(physical_free_bytes, "
            "allocator_limit_bytes - allocator_used_bytes) // 2"
        ),
        "headroom_divisor": 2,
        "units": {
    ...
    try:
        ...
        raise ValueError(
            f"Cannot configure the Mahler GPU workload: {error}"
        ) from error
    else:
...
def _configure_observed_devices(
    *,
    ...
    for device in devices:
        ...
        observation["memory_stats"] = device.memory_stats()
...
def _timestamp() -> str:
    """Return the UTC observation time."""
...
def _load_cuda_driver() -> ctypes.CDLL:
    """Use CUDA's own visibility mapping for ordinal-to-UUID identification."""
...
def _get_cuda_uuid(*, driver: ctypes.CDLL, ordinal: int) -> str:
    """Resolve a JAX CUDA ordinal through its driver handle and 16-byte UUID."""
    handle = ctypes.c_int()
...
def _mib_bytes(*, value: str, name: str) -> int:
    """Convert NVIDIA's integral MiB observation to bytes."""
