...
def test_the_smallest_subnormal_is_not_enlarged_in_a_linear_continuation() -> None:
    ...
    got = np.longdouble(
        np.asarray(
            _source_value(
                model=_model(
                    rare_probability=_smallest_subnormal,
                    rare_payoff=_largest_finite_rare_payoff,
                )
            )
        )
    )
...
def test_the_smallest_subnormal_keeps_its_true_nonlinear_weight() -> None:
    ...
    got = jnp.asarray(model.solve(params=params, log_level="off").values[0]["source"])
...
def test_a_negative_smallest_subnormal_does_not_bypass_the_distribution_guard() -> None:
    ...
    model = _model(
        rare_probability=lambda: -_smallest_subnormal(),
        rare_payoff=_largest_finite_rare_payoff,
    )
    value = _source_value(model=model, log_level="off")
