"""BQSEGM solves against a budget node named other than the convention `coh`.
...
`BQSEGM(budget_target="resources")` must reproduce the dense-grid `GridSearch`
...
from tests.test_models import bqsegm_tax_toy as toy
...
def test_bqsegm_with_renamed_budget_target_matches_brute_every_age():
    """A non-`coh` budget node solved by BQSEGM equals brute at every working age."""
