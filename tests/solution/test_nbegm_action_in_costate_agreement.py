"""NBEGM carries a branch-specific continuation when an action feeds a co-state.
...
not one shared across branches. NBEGM's value function must match a dense brute
...
def test_nbegm_action_in_costate_matches_brute() -> None:
    ...
    # node x streak co-state must agree across the liquid interior. The streak
