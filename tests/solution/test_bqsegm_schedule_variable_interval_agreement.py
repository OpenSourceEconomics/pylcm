"""BQSEGM partitions per branch even when the continuation reads the liquid state.
...
continuation core and the envelope must use each branch's own breakpoints. BQSEGM's
...
def test_bqsegm_schedule_variable_interval_matches_brute() -> None:
