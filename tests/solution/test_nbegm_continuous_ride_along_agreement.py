"""NBEGM agreement on a derived-income subsidy cliff with a continuous co-state.
...
all of which the NBEGM schedule solver must treat as ride-along axes integrated
...
so the savings-space jump step is exact: NBEGM reproduces the dense `GridSearch`
...
def test_nbegm_continuous_ride_along_regime_builds() -> None:
    ...
    solved = model.solve(params=toy.build_params(), log_level="debug").values
    period = _terminal_adjacent_period(solved)
    assert np.asarray(solved[period]["alive"]).shape == (_N_LIQUID, _WAGE.size)


def test_nbegm_continuous_co_state_matches_brute_across_the_cliff() -> None:
    """The jumped derived-income solve equals brute across every wage slice's cliff.

    At the terminal-adjacent working period the continuation is the smooth bequest,
    so the savings-space jump step is exact: NBEGM matches the dense `GridSearch`
