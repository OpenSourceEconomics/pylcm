...
it never inserts the exact segment-crossing abscissa that FUES duplicates into
its refined rows. A kink therefore lands between two retained RFC points
instead of exactly on the crossing, so the downstream Hermite carry read
recovers it to within the local grid spacing — a second-order error given the
exact-slope ($\\mu = \\partial v / \\partial R$) supgradient reads. On a
concave segment (no crossing) the two backends agree to machine precision; the
tolerance only absorbs the kink-placement delta where a discrete choice
switches. Where the two backends meet brute-force-unstable low-wealth nodes,
those nodes are excluded exactly as the underlying FUES tests do.
"""
...
# cubically clustered savings grid the retirement battery uses, that bounds the
# per-node V difference between the backends well inside this tolerance.
_PARITY_ATOL = 5e-3
_PARITY_RTOL = 1e-3
...
def test_rfc_matches_fues_on_concave_retirement(n_periods):
    """The pure-concave retirement solve agrees between RFC and FUES.
    ...
    for period in sorted(fues)[:-1]:
        np.testing.assert_allclose(
            np.asarray(rfc[period]["retirement"]),
...
def test_rfc_publishes_neg_inf_for_all_infeasible_combo_like_fues():
    """An all-infeasible worker regime publishes `-inf` V under RFC too.

    A discrete-only constraint false everywhere makes the worker's value `-inf`
    at every state; the dead candidates must stay deleted from the dominance
    test (`-inf`/NaN poisoning discipline), so RFC publishes `-inf` exactly as
    FUES does, never NaN.
    """
    n_periods = 4
    ...
    for period in sorted(rfc)[:-1]:
        ...
        assert bool(np.isfinite(rfc[period]["retirement"]).all())


@pytest.mark.parametrize("n_periods", [4])
def test_rfc_matches_fues_on_discrete_choice_working_life(n_periods):
    """The work/retire discrete-choice solve agrees between RFC and FUES.

    The labour-supply choice creates a non-concave kink in the worker's value
    correspondence. FUES inserts the exact crossing; RFC retains the bracketing
    points and lets the Hermite carry recover the kink. The published value
    functions agree within the no-insertion tolerance on the wealth nodes where
    ...
    for period in sorted(fues)[:-1]:
        for regime in ["working_life", "retirement"]:
            np.testing.assert_allclose(
                np.asarray(rfc[period][regime])[..., n_unstable_low_nodes:],
...
def test_rfc_handles_discount_factor_zero_like_fues():
    """The β=0 consume-everything degenerate solves identically under RFC.

    With a zero discount factor every savings node's marginal continuation is
    zero, so the consume-everything corner wins at every wealth node. Both
    backends must publish the same value row.
    """
    n_periods = 3
