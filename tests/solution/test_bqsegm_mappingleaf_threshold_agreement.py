"""BQSEGM agreement when a cliff threshold lives inside a `MappingLeaf` param.
...
FPL schedules. The BQSEGM ride-along solver must follow the
...
from tests.test_models import bqsegm_mappingleaf_threshold_toy as toy
...
def test_bqsegm_mappingleaf_threshold_matches_brute_across_each_cliff():
    ...
    smooth bequest, so the savings-space jump step is exact: BQSEGM matches the
