"""BQSEGM partitions each branch on its own breakpoints when an action shifts them.

A binary discrete choice enters the derived income the tax cliff is measured against,
so the cliff's asset (liquid) preimage differs per branch — each branch needs its own
breakpoint partition, not one shared across branches. BQSEGM's value function must match
a dense brute solve across the liquid interior, over the income ride nodes.
"""
...
def test_bqsegm_action_in_schedule_variable_matches_brute() -> None:
    """`V` agrees with a 1200-point brute across the liquid interior when the
    discrete action shifts the schedule variable (a per-branch breakpoint partition),
    over the income nodes."""
