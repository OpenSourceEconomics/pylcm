...
def test_the_epstein_zin_partials_come_down_by_the_same_scale() -> None:
    """Only mass and the marginal log scale carry the common weight scale."""
    dtype = _active_dtype()
    shift = jnp.asarray(3, jnp.int32)
    certainty_equivalent, weight_sum, marginal_log_scale, marginal_mantissa = (
        jnp.asarray(v, dtype=dtype) for v in (0.5, 8.0, 1.0, 0.25)
    )
    ...
    np.testing.assert_allclose(
        float(lowered[2]), 1.0 - 3.0 * float(np.log(2.0)), rtol=1e-6
    )
    assert float(lowered[3]) == 0.25
...
def test_an_unlifted_mesh_leaves_the_epstein_zin_partials_alone() -> None:
    ...
    assert [float(v) for v in lowered] == [0.5, 8.0, 1.0, 0.25]
