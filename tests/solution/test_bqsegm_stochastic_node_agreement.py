"""BQSEGM with a stochastic ride-along income node: brute agreement + splay invariance.
...
#   liquid grid: brute edge-clamps its next-period V lookup while BQSEGM extrapolates
...
#   BQSEGM machinery is internally consistent there).
...
def test_bqsegm_stochastic_node_matches_dense_brute_force():
    """BQSEGM integrating a stochastic income node matches dense brute force.

    The child income node is distributed; the BQSEGM continuation weights the per-node
    ...
    """
    bqsegm_solution = _solve("bqsegm")
    brute_solution = _solve("brute")
    for period in sorted(brute_solution)[:-1]:
        brute_V = np.asarray(brute_solution[period]["alive"])
        bqsegm_V = np.asarray(bqsegm_solution[period]["alive"])
        assert brute_V.shape == bqsegm_V.shape
...
def test_bqsegm_stochastic_node_batch_size_leaves_value_function_unchanged(
    ...
    """
    reference = _solve("bqsegm", stochastic_node_batch_size=0)
    splayed = _solve("bqsegm", stochastic_node_batch_size=stochastic_node_batch_size)
