"""NBEGM agreement when the period utility is CES, not plain CRRA.
...
recovers the wrong action. The NBEGM ride-along solver must invert the *model's*
marginal utility, whether the NBEGM ride-along solver inverts the
...
savings-space step is exact: NBEGM reproduces the dense `GridSearch` value across the
...
def test_nbegm_ces_utility_matches_brute(breakpoint_kind: str) -> None:
    ...
    from the period utility), NBEGM matches the dense `GridSearch` value across the
