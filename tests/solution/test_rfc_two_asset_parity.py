...
def test_rfc_two_asset_tracks_brute_on_the_covered_region():
    """The RFC published value tracks the brute solve where the grid covers."""
    rfc_value, brute = _solve()
    assert np.isfinite(rfc_value[_COVERED]).all()
    rel = np.abs(rfc_value[_COVERED] - brute[_COVERED]) / np.abs(brute[_COVERED])
