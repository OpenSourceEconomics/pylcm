"""BQSEGM agreement with brute under a stochastic multi-target transition.

A living BQSEGM regime whose lifecycle transition reaches two distinct living
...
from tests.test_models import bqsegm_multi_target_toy as toy
...
def test_bqsegm_matches_brute_under_stochastic_multi_target_transition():
    ...
    for period in brute:
        for regime in ("alive_a", "alive_b"):
            if regime not in brute[period] or regime not in bqsegm[period]:
                continue
            brute_v = np.asarray(brute[period][regime])
            bqsegm_v = np.asarray(bqsegm[period][regime])
