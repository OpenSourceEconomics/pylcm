"""BQSEGM carries a branch-specific continuation when an action feeds the regime transition.
...
and must read its own blended next-period value. BQSEGM's value function must match a
...
def test_bqsegm_action_in_regime_transition_matches_brute() -> None:
