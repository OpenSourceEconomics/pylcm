"""Normal endpoints whose exact separation is subnormal keep their strict order.
...
states that as a *property* of the inputs rather than as a construction:

- every abscissa handed to the certificate is normal by its exponent bits;
- the exact difference between two endpoints of the same link is a positive
  subnormal, so a backend that flushes subnormals loses the whole distance.

Witnesses are reached by stepping raw bit patterns across the bottom binades,
and the ground truth is computed in exact rational arithmetic, so neither the
witness nor the oracle shares machinery with the predicate under test.
"""
