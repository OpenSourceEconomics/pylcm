...
def test_rfc_benchmark_reports_sane_time_and_euler_errors():
    """The RFC row has a positive solve time and finite interior Euler errors."""
    row = benchmark_rfc_ds_pension(n_liquid=12)
    assert row.method == "RFC"
    assert row.solve_seconds > 0.0
    assert row.euler_error_median_log10 < -1.0  # better than 10% on the interior
    assert row.euler_error_p90_log10 < row.euler_error_median_log10 + 1.0


def test_methods_runner_returns_a_g2egm_and_an_rfc_row():
    """The comparison runner emits one G2EGM and one RFC row at the same resolution."""
    rows = benchmark_ds_pension_methods(n_liquid=12)
    assert [r.method for r in rows] == ["G2EGM", "RFC"]
    assert all(r.n_liquid == 12 for r in rows)
