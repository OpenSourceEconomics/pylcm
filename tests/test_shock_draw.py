...
def test_draw_shock_uniform(params_at_init):
    ...
    if params_at_init:
        grid = lcm._processes.iid.Uniform(
            n_points=5, batch_size=0, distributed=False, **kwargs
        )
        params = grid.params
    else:
        grid = lcm._processes.iid.Uniform(n_points=5)
...
def test_draw_shock_lognormal(params_at_init):
    ...
    if params_at_init:
        ...
        grid = lcm._processes.iid.LogNormal(n_points=5, gauss_hermite=True)
        params = MappingProxyType(kwargs)
    draws = _draw_many(grid, params)
...
def test_draw_shock_tauchen(params_at_init):
    ...
    if params_at_init:
        ...
        grid = lcm._processes.ar1.Tauchen(n_points=5, gauss_hermite=True)
        params = MappingProxyType(kwargs)
...
def test_draw_shock_rouwenhorst(params_at_init):
    ...
    if params_at_init:
        ...
        grid = lcm._processes.ar1.Rouwenhorst(n_points=5)
        params = MappingProxyType(kwargs)
    draws = _draw_many(grid, params, current_value=3.0)
    aaae(draws.mean(), 3.5, decimal=1)
    aaae(draws.std(), 0.1, decimal=1)
...
def test_draw_shock_normal_mixture(params_at_init):
    ...
    if params_at_init:
        ...
        grid = lcm._processes.iid.NormalMixture(n_points=5)
        params = MappingProxyType(kwargs)
    draws = _draw_many(grid, params)
    p1 = kwargs["p1"]
