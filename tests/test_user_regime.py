...
def test_state_name_does_not_contain_separator():
    with pytest.raises(RegimeInitializationError, match=QNAME_DELIMITER):
        UserRegime(
            ...
            transition=next_wealth,
            active=lambda age: age < 5,
            state_transitions={f"my{QNAME_DELIMITER}wealth": None},
        )
...
def test_stochastic_markov_state_with_batch_size_is_rejected():
    """A stochastic (Markov) state carrying `batch_size > 0` is rejected.

    `batch_size` on a stochastic state chunks the next-period shock-integration
    map, which de-fuses the expected-continuation-value reduction and increases
    peak memory rather than reducing it. The regime is rejected at construction,
    pointing the user to shard the current axis or splay a continuous state.
    """
    ...
    with pytest.raises(RegimeInitializationError, match=r"stochastic.*batch_size"):
        UserRegime(
            transition=lambda: 0,
            functions={"utility": lambda health: health},
            states={"health": DiscreteGrid(Health, batch_size=1)},
            state_transitions={"health": MarkovTransition(func=lambda health: health)},
        )


def test_stochastic_markov_per_target_dict_with_batch_size_is_rejected():
    """A per-target Markov transition with `batch_size > 0` is rejected too.

    The de-fusion is independent of whether the stochastic transition is a plain
    `MarkovTransition` or a per-target dict of them, so both are caught.
    """
    ...
    with pytest.raises(RegimeInitializationError, match=r"stochastic.*batch_size"):
        UserRegime(
            transition=lambda: 0,
            functions={"utility": lambda health: health},
            states={"health": DiscreteGrid(Health, batch_size=2)},
            state_transitions={
                "health": {
                    "working_life": MarkovTransition(func=lambda health: health),
                },
            },
        )


def test_non_stochastic_discrete_state_with_batch_size_is_accepted():
    """A deterministic discrete state may carry `batch_size > 0`.

    On a non-stochastic state, `batch_size` splays the state-action axis, which
    only reduces peak memory — so it must remain valid.
    """
    ...
    regime = UserRegime(
        transition=lambda: 0,
        functions={"utility": lambda education: education},
        states={"education": DiscreteGrid(Edu, batch_size=1)},
        state_transitions={"education": lambda education: education},
    )
    assert regime.states["education"].batch_size == 1
