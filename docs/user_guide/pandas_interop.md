...
agent, one column per state variable, plus a `"regime_id"` column:

```python
df = pd.DataFrame(
    {
        "regime_id": ["working", "working", "retired"],
...
- `"regime_id"` column is required. Use regime names as strings (e.g., `"working"`).
