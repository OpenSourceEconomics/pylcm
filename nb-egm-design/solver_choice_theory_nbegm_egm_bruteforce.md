...
**Expanded Version 3: revised after adversarial math/code audit; clarifies the interpolation target, topology-preserving publication, endpoint ownership, NaN-dead masks, validator scope, and the full NB-EGM cost inequality.**
...
**Purpose:** Provide a publication-quality decision and proof framework for when brute force, standard EGM, DC-EGM/FUES, query-side segmented EGM, NB-EGM, NEGM, G2EGM, RFC, and related pylcm implementations are possible, chunkable, GPU-friendly, and optimal.
...
5. Formal NB-EGM specification
6. NB-EGM correctness theorems
...
- \(N_C\): number of NB-EGM case assignments or case combinations that are actually solved after any static pruning;
...
| **Segment** | Solver-internal smooth one-dimensional EGM object or explicit envelope piece; in NB-EGM, segment identifiers are assigned per monotone feasible subsegment, not merely per case. |
...
NB-EGM is best read as **case-piece query-side EGM**. The model-facing concepts are
**case boundaries** and **pieces**. The solver-facing objects are **segments** with
explicit topology.

NB-EGM is not implemented in PR #390 as a named solver. PR #390 supplies useful
building blocks, especially query-side envelope evaluation with `segment_id`, but a full
NB-EGM solver also needs endpoint ownership metadata, NaN-dead masking, and a
...
Given \(\sigma\), NB-EGM constructs a specialized DAG \(G_\sigma\) by replacing each
...
### 5.6 NB-EGM candidate generation
...
For every declared boundary \(h_{p,\ell}(x)=0\), NB-EGM must either:
...
segments with `segment_id` topology. A complete NB-EGM implementation must either extend that
...
NB-EGM may publish value, policy, and marginal on a regular query grid, but exact backward
...
Suppose further that for each \(\sigma\), the NB-EGM generator produces exactly the segments
...
maximum over the union of all eligible generated records. Then NB-EGM returns the exact value
...
### Theorem 2: NB-EGM restores standard-EGM amortization when case conditioning removes current-state dependence
...
### Theorem 3: Hidden piecewise logic makes exact NB-EGM treatment impossible from finite black-box samples
...
**Corollary.** NB-EGM requires exposed `case_boundary` metadata for exact threshold handling.
...
### Theorem 7: NB-EGM dominates asset-row EGM only under an explicit cost inequality
...
and let NB-EGM have total cost
...
NB-EGM is faster than asset-row EGM if and only if
...
**Implication.** ACA-like models benefit from NB-EGM only if the full inequality is favorable:
...
### 9.3 When NB-EGM fails for ACA

NB-EGM fails if, even after all finite cases are fixed,
...
### 9.4 Critical validator for ACA-style NB-EGM

A NB-EGM implementation must reject hidden economic case logic inside user-authored smooth
...
- treat the existing numerical cliff checker as a savings-stage/node-resolution diagnostic, not
  as a complete substitute for NB-EGM case-piece validation.
...
| Finite institutional thresholds | NB-EGM |
...
2. run EGM/NB-EGM/NEGM at increasing post-decision grid density;
...
1. **Case and segment explosion.** NB-EGM can become as bad as asset-row mode if the case product or the monotone-subsegment count is large.
...
4. **Topology publication.** A plain aggregate carry can bridge across switches; NB-EGM needs a topology-preserving payload or a switch-refined aggregate grid.
...
If yes, NB-EGM is the natural solver. If no, brute force or asset-row EGM remains the honest choice.
