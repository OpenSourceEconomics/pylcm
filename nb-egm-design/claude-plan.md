# PLAN: Implement NB-EGM (case-piece EGM) off `feat/dcegm`

## Implementation status (branch `feat/nbegm`) — COMPLETE
...
- **B1** — `lcm.case_boundary` / `piece` / `boundary` / `smooth_helper` decorators
  (metadata-only, claw-safe) + `NB-EGMCaseError`.
- **B2** — `_lcm.egm.nbegm` lowering: metadata collection, strict coverage / boundary
  checks, per-case variant producer-maps.
- **B3** — `_lcm.egm.nbegm_validation` AST + JAXPR smoothness gate scoped to
  user-economic nodes.
- **B4a** — `_lcm.egm.nbegm_segments`: NaN-dead masking + fold/hole `segment_id`.
- **B4b** — endpoint-aware `envelope_at_query` (in-place, default-closed; RT1 green;
  existing envelope suite unchanged).
- **B4-core** — `NBEGM` solver (`lcm.solvers.NBEGM`) + `_lcm.egm.nbegm_step` for the
...
- Design docs (v2, in this dir) — `solver_choice_theory_nbegm_egm_bruteforce.md` and
  `nbegm_case_piece_api_design.md` — **rewritten** after a ChatGPT-5.5-Pro adversarial
...
**NB-EGM** = *Branchwise/Casewise Query-Side EGM*: the next solver after the DC-EGM /
...
(`solution/contract.py`). So NB-EGM needs **three real extensions**, not just "light up
`segment_id`":
...
### B0. Base & branch
Branch `feat/nbegm` off `origin/feat/dcegm` tip. Stacked-PR workflow: increments as commits
...
- **Numeric check (diagnostic, Pro F4):** the existing `egm/validation.py` grid+midpoint
  ...
  build-time-evaluable**; it does **not** by itself establish NB-EGM piece smoothness. The
...
- New `class NB-EGMCaseError(PyLCMError)` in `src/lcm/exceptions.py`.
...
### B4. NB-EGM solver — candidate generation + NaN-dead masking + boundary candidates (Pro F1, A6)
- New `NBEGM` solver subclassing `Solver` (`solution/contract.py`), registered like `DCEGM`
...
- **RED test = the Pro-supplied RT1** (`test_nbegm_envelope_endpoint_contract.py`):
...
- `tests/test_models/nbegm_medicaid_toy.py`: `build_model(variant="nbegm"|"brute", ...)` +
...
- `tests/solution/test_nbegm_medicaid_brute_agreement.py`: solve both,
...
- Memory-vs-asset-row check (NB-EGM peak transient < asset-row for the same toy).
...
### B6. Docs + AGENTS.md
- `docs/explanations/nbegm.ipynb` (mirror an existing notebook); AGENTS.md solver + decorator
  + `boundary(...)` entries.
...
brute-vs-NB-EGM toy agreement (global + threshold-local); the five acceptance tests; memory
check.
...
3. **Branch & sequencing** — `feat/nbegm` off `origin/feat/dcegm` tip (`c385d25`).
