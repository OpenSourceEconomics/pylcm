# NB-EGM Case-Piece API Design for pylcm
...
**Goal:** Let NB-EGM exploit piecewise institutional rules without adding a new object-oriented modeling layer, while preserving boundary topology, validator scope, and the exact interpolation convention needed by EGM.

---

## 1. Motivation

NB-EGM needs to solve EGM problems branchwise when institutional rules create formula changes, kinks, cliffs, or notches. In models such as ACA, rules like Medicaid eligibility, subsidy brackets, premium default, and consumption floors are often written naturally as `if` statements, lookup tables, or `jnp.where` expressions.

Those are fine for brute-force evaluation, but opaque to an EGM-style solver. NB-EGM needs to know:
...
and that equality belongs to the `otherwise` side for both surfaces. NB-EGM uses this metadata
...
BruteForce can use this normal DAG node. NB-EGM can ignore it and lower the decorated pieces directly.
...
NB-EGM internally creates two smooth DAG variants:
...
The public API should not expose `dags.tree` to model authors. Internally, NB-EGM should use
...
NB-EGM relies on formulas being smooth within each piece. Therefore, validation must guarantee:

```text
No hidden economic case logic exists in any user-authored smooth NB-EGM subgraph.
```
...
NB-EGM smooth variant. Solver-provided continuation interpolation may contain search/select
...
class PiecewiseASTChecker(ast.NodeVisitor):
    ...
    def visit_If(self, node):
        self.error(node, "Python if is not allowed in NB-EGM-traced functions.")
            ...
            errors.append(
                {
                    ...
                    "message": (
                        ...
                        "inside a smooth NB-EGM formula. Expose it as a case "
...
By contrast, a NB-EGM continuation read implemented by pylcm may contain search or select
...
a NB-EGM smooth variant, and explicitly stop at trusted solver-provided continuation and
envelope boundaries.

Algorithm:

```text
for each NB-EGM regime:
    build ordinary DAG
...
If the fallback function `oop` contains `jnp.where`, but NB-EGM replaces it with
...
arguments are free parameters at build time. NB-EGM may reuse or extend it, but the API should
...
- every boundary is compatible with the state/action variables used by NB-EGM;
...
Multiple predicates create many case combinations. Initially, NB-EGM can enumerate and mask. Later it should prune impossible combinations statically when predicates logically contradict each other.
...
13. Compare NB-EGM against brute force on the toy under the same interpolation/envelope convention.
...
- brute force and NB-EGM agree on a dense-grid toy under the same convention;
- topology-preserving publication avoids aggregate interpolation bridges;
- NB-EGM beats asset-row mode on memory for the same toy only when the full cost inequality is favorable.
...
NB-EGM should expose institutional piecewise structure through metadata on ordinary DAG
functions:
